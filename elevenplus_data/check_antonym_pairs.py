#!/usr/bin/env python3
"""
Independent checks on the Paired Antonyms pack (contrib_prash_vr_22.json).

    python3 elevenplus_data/check_antonym_pairs.py elevenplus_data/contrib_prash_vr_22.json
...
cross-bracket combinations is a genuine pair of opposites is a semantic
...
Mechanically:
  ...
  * NO PAIR TWICE, IN EITHER DIRECTION. The key pair is compared as an
    unordered pair across the whole pack, so asking "victory/defeat" and
    "defeat/victory" as two questions is caught — that is the same fact twice
    and the reason this pack is 11 questions and not 22.
  * THE BAND IS RE-DERIVED from the rarer word's frequency, because the pool's
    own bands are positional slices (`[0:3]`, `[3:7]`, `[7:11]`) whose measured
    gradient is partly inverted. Without `wordfreq` this check reports itself
    skipped.
...
PACK = sys.argv[1] if len(sys.argv) > 1 else "elevenplus_data/contrib_prash_vr_22.json"
...
print("Exactly one line per question should be a real pair of opposites:\n")
...
for q in qs:
    ...
    if q.get("subtopic") != "Paired Antonyms":
    ...
    if WORDS is not None:
        for w in words:
            ...
            if w.lower() not in WORDS or zipf_frequency(w.lower(), "en") < 2.4:
