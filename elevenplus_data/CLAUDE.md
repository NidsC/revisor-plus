...
- **Figures generated from data — except tables.** A shared data table can be declared with
  `tables` and is drawn server-side (see "Sharing a data table"). Charts and diagrams cannot:
  `image` takes a committed file, and there is no way to declare one and have it drawn.
